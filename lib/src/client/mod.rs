...
use libp2p::swarm::{CloseConnection, ConnectionHandler, IntoConnectionHandler, NetworkBehaviour, NetworkBehaviourAction, NotifyHandler, PollParameters};
...
impl ProxyClient {
    ...
    pub fn connect(&mut self, local_addr: SocketAddr, remote_addr: SocketAddr, peer_id: PeerId) {
        let _ = self.event_loop.send_event(IncomingProxyClientEvent::OpenConnection {
            local_addr,
            remote_addr,
            peer_id
        });
...
impl NetworkBehaviour for ProxyClient {
    ...
    fn inject_connection_established(&mut self, peer_id: &PeerId, connection_id: &ConnectionId, _endpoint: &ConnectedPoint, _failed_addresses: Option<&Vec<Multiaddr>>, _other_established: usize) {
        let _ = self.event_loop.send_event(
            IncomingProxyClientEvent::HandlerConnected(peer_id.clone(), connection_id.clone())
        );
    }

    fn inject_event(&mut self, peer_id: PeerId, _: ConnectionId, event: <<Self::ConnectionHandler as IntoConnectionHandler>::Handler as ConnectionHandler>::OutEvent) {
        let _ = self.event_loop.send_event(
            IncomingProxyClientEvent::HandlerEvent(
...
async fn listen_for_connections(peer_id: PeerId, local_addr: SocketAddr, sender: Sender<(PeerId, TcpStream)>) -> io::Result<()> {
...
impl RunningLoop {
    ...
    fn send_event(&self, event: IncomingProxyClientEvent) -> Result<(), SendError<IncomingProxyClientEvent>> {
        block_on(self.local_event_sender.send(event))
    }

    fn poll_event(&mut self, cx: &mut Context<'_>) -> Poll<NetworkBehaviourAction<ProxyClientEvent, ProxyClientHandler>> {
...
impl EventLoop {
    ...
    async fn process_event(&mut self, event: IncomingProxyClientEvent, sender: Sender<(PeerId, TcpStream)>) -> Result<(), Box<dyn Error>> {
        match event {
            IncomingProxyClientEvent::OpenConnection { local_addr, remote_addr, peer_id } => {
                let f = spawn(listen_for_connections(peer_id.clone(), local_addr.clone(), sender.clone()));
                let con = Connection {
                    listener: f,
                    local_addr,
                    remote_addr,
            ...
            IncomingProxyClientEvent::Stop(peer_id) => {
                if let Some(con) = self.handlers.remove(&peer_id) {
                    self.event_sender.send(NetworkBehaviourAction::CloseConnection {
                        peer_id,
            ...
            IncomingProxyClientEvent::HandlerEvent(e) => {
                let _ = self.event_sender.send(NetworkBehaviourAction::GenerateEvent(e)).await;
            }
        }
        Ok(())
    }
        ...
        loop {
            ...
            if let Some((peer_id, stream)) = pending_streams.pop_front() {
                if let Some(connection_id) = self.handlers.get(&peer_id) {
                    let _ = self.event_sender.send(NetworkBehaviourAction::NotifyHandler {
                        peer_id,
